  ...
  <div class="flex justify-between border-b-2 h-60">
    <div class="flex h-full">
      <div
        ...
        class="flex h-full cursor-pointer w-250 border-r-2 relative"
      >
        ...
        <ChevronDownIcon
          v-if="isChevronDown"
          class="h-7 w-7 cursor-pointer text-gray-600"
        />
        <ChevronUpIcon
          v-if="!isChevronDown"
          class="h-7 w-7 cursor-pointer text-gray-600"
      ...
      <div
        v-if="!isAddingNewBoard"
        class="flex justify-center items-center border-r-2 h-full w-70"
      >
      ...
      <div
        v-if="isAddingNewBoard"
        class="flex h-full w-300 border-r-2 px-2 bg-gray-100"
      >
        <input
          v-on:keyup.enter="addDashboard"
          ref="dashboardInput"
          v-model="dashboardInputModel"
          class="focus:outline-none text-left p-2 ml-2 w-250 bg-transparent"
...
export default {
  ...
  data: () => {
    return {
      ...
      dashboardNames: ["Key Result Board"],
